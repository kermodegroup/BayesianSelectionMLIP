...
def select_hal_samples(dataset, basis_args, N_structs, core_ds=None, N_samples=1, score_func=hal_force, iso_atom_config_type=None, 
    ...
    solver_kwargs["E0s"] = E0s
    solver_kwargs["B_len_norm"] = define_basis(basis_args)
    ...
    for isamp in trange(N_samples):
        ...
        committee = fit(ds, solver, pot_file=None, **solver_kwargs)
        ...
        for istruct in trange(np.max(N_struct), leave=False):
            ...
            committee, samp_structs, ds, solver = hal_step(committee, samp_structs, dataset, solver, score_func=hal_force, model_fname=fname + ".json", {}, solver_kwargs):
